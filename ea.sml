...
= struct
    ...
    in 
	   ...
	   = Stack.push (Source_syntax.LIT n, s)
	 | decode_execute (ADD, s)
	   = let val (e2, s1) = Stack.pop s
		 val (e1, s2) = Stack.pop s1
	     in Stack.push (Source_syntax.OPR (e1, Source_syntax.PLUS, e2), s2)
	     end
	 | decode_execute (SUB, s)
	   = let val (e2, s1) = Stack.pop s
		 val (e1, s2) = Stack.pop s1
	     in Stack.push (Source_syntax.OPR (e1, Source_syntax.MINUS, e2), s2)
	     end
	 | decode_execute (MUL, s)
	   = let val (e2, s1) = Stack.pop s
		 val (e1, s2) = Stack.pop s1
	     in Stack.push (Source_syntax.OPR (e1, Source_syntax.TIMES, e2), s2)
	     end
...
= struct
    ...
    local open Source_syntax
    in
       val s2 = OPR (OPR (LIT 30, TIMES, OPR (LIT 20, PLUS, LIT 40)),
		     TIMES,
