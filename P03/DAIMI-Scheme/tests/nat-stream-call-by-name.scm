...
(define make-stream
  (lambda (seed next)
    (letrec ([loop (lambda (seed)
		     (cons seed (suspend (loop (next seed)))))])
      (loop seed))))
...
(define merge-streams
  (lambda (s1 s2 predicate)
	...
	(cons (stream-car s1)
	      (suspend (merge-streams (stream-cdr s1) s2 predicate)))
	(cons (stream-car s2)
	      (suspend (merge-streams s1 (stream-cdr s2) predicate))))))
